...
A CHE SERVE IL CTX qui?

\subsubsection{Notes on the CHR semantics and the alignment expression}
...
... hence we provide the dojoint union alignment

and we expose the explicit substitution...
...
\subsection{Meta level = lp + matching}

The CHR is expensive, freezing/unfreezing is expensive and in many cases
the multi-goal maching facility is not needed.  In particular

$S_1 \setminus S_2 ~|~ G \iff S_3$ whose
declarative semantics is that $\bigwedge S_1 \wedge G \Rightarrow (\bigwedge
S_2 \iff \bigwedge S_3)$ is a rewriting that looses no solution, but
then only this holds
 $\bigwedge S_1 \wedge G \Rightarrow (\bigwedge
S_2 \Leftarrow \bigwedge S_3)$
the rule can be seen as an heuristic.  (still useful and even wanted in an
elaborator).
...
\begin{verbatim}
conv (?? as X L) T :- heuristic-assignment L T X.
\end{verbatim}

This shall be useful in the elaborator, e.g. to craft an assignment for
\verb+X+ when \verb+L+ is not in L$_\lambda$.

This ``shortcut'' is interpreted as a

\begin{verbatim}
delay (conv X T) on X.
constraint conv {
  rule \ (conv (?? X L) T) | (heuristic-assignment L T V) <=> (X = V). 
\end{verbatim}
