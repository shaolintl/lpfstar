...
explicitly substituted existential variables (now open-terms)
...
In particular one no more works with terms, but always with open terms and
...
Given such complexity in the elaborator, and the safety requirements of interactive provers, the kernel of the system is kept simple by making it unaware of open terms.  As a consequence a lot of code is duplicated, and
...
starting from a meta typed
with the conjecture, the proof commands make progress by instantiating such
meta with open terms.  Once there are no unresolved meta variables left,
the ground term is checked by the kernel.
...
ad-hoc extension points to extend the capabilities of the elaborator.  The
languages to write this code are typically high level, delarative programs,
that hide the user from the intricaces of bound variables, metavariables, etc.
The global algorithm is therefore split in multiple languages, defying the hope
for static analysis, documentation, and sometimes predictability.
\end{description}
...
\item The programming language should take care of representation of
	bindings \emph{and metavariables} in the spirit of semi shallow embedding~\cite{Dunchev:2016:IHH:2966268.2966272}.
	This would solve the platform issue.
	It would also improve on extensibility  issues by allowing
	both the core implementors and the users to work on the same,
	high level code.
	Finally the intricacy of elaboration would be mitigated.
\item The programming language should feature a primitive and powerful
	notion of  extensibility.  
	In this way the rules of the kernel should not be re-implemented in the
	elaborator. On the contrary they could be extended to cover open terms,
	solving the code duplication issue.
	By following this approach the
	twins' disagreement problem should become less severe.
	Also the extensibility of the elaborator would be less ad-hoc.
\item Finally the
	language should make it easy to prove that the additional
	rules grant that the elaborated terms are well-typed according
	to the core set of rules of the kernel.  This would completely
	solve the twins' disagreement problem, making it possible to
	eventually merge the kernel and the elaborator.
\end{enumerate}

We envisage such language to be a logic one for two reasons:
first we hope to reuse or at least extend $\lambda$Prolog;
second we observe that the third layer of an ITP, the ones of proof commands,
is better implemented on a language where backtracking is built in, in
particular when proof commands perform proof search.

$\lambda$Prolog already has existentially quantified metavariables and the
related operations. Therefore, in order to satisfy requirement 1) it seems
sufficient to use a shallow embedding of the syntax: 
\emph{the metavariables of the
partial terms are directly encoded as metavariables of $\lambda$Prolog},
similarly to how binders are encoded via the $\lambda$-abstractions of
$\lambda$Prolog. 

Does this idea work out of the box? Unfortunately not at all.
...
This is not a problem specific of $\lambda$Prolog, also Prolog has it.  Indeed
all modern Prolog engines provide a \verb+var/1+ built-in to test/guard
predicates against flexible input, provide one/many variants of \verb+delay/2+
to suspend a goal until the input becomes rigid, and provide modes declarations
to both statically/dynamically detect problematic calls to be
(semi)automatically delayed. These mechanisms, however, have never been
standardized.

For example, by using the \verb+delay+ pack of SWI-Prolog~\cite{???}, the goal
...
One could imagine to provide

\begin{verbatim}
delay (of X T) on X.
\end{verbatim}

and end up with the following suspended goal

\begin{verbatim}
of x (sort I) ?- of (P x) (arr x _\ x)
\end{verbatim}

that is a sort of constraint on \verb+P+ : when it gets instantiated
the goal is resumed and the assigned term is checked to fix the type.

Extending lp with delay could make it possible to represent the metasenv (as
delayed constraints) and avoid the generative semantic on predicates that, like
\verb+of+ are not generative.
