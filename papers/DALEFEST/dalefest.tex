...
We already gave a positive answer in~\cite{us} where we implemented in
$\lambda$Prolog a type-checker for Landau's Grundlagen XXXXXXXX.
In Section~\ref{sec:kernel} we confirm the answer again by implementing a kernel for a generic PTS that supports cumulativity between sorts as well. We then instantiate the PTS to the one of Matita and we modularly add to the kernel support for globally defined constants and form primitive inductive types, following the variant of the Calculus of Inductive Constructions of Matita. Finally, we interface the implementation with the code of Matita to test it on the arithmetic library of the system.
...
We start identifying syntactically types and terms, adding a new constructor for sorts of the PTS and finally refine \verb+arr+ to the dependently typed product.
...
expected for the argument of an application are meant to be compared up to $\beta$-reduction and cumulativity of universes. The $\verb+sub+$ predicate does the check. The \verb+conv+ predicate is used instead to compare two terms up to $\beta$-reduction only. Finally, the \verb+succ+ and \verb+max+ predicates are meant to be later instantiated with the corresponding rules of a PTS.
...
of (app M N) BN :- of M TM, conv TM (arr A1 Bx), of N A2, sub A2 A1, BN = Bx N.
...
In order to implement \verb+conv+ and \verb+sub+, we first implement weak head
reduction in one step \verb+whd1+ and weak head normalization \verb+whd*+.
The former is trivial because we reuse the $\beta$-reduction of $\lambda$Prolog for capture avoiding substitution:
...
The conversion and subtyping tests are a bit more delicate, since performance-wise they are critical piece of code (see section~\ref{sec:kernel}). Here we code
the usual strategy of normalizing terms only if really necessary (what
systems typically do since Automath). The two judgements are generalized to a single \verb+comp T1 D T2+ predicate that behaves as \verb+conv T1 T2+ when \verb+D+ is \verb+eq+, and as \verb+sub+ if \verb+D+ is \verb+leq+.
...
\subsection{An efficient kernel implementation}\label{sec:kernelmachine}
...
\section{Prototype elaborator}\label{sec:elaborator}
