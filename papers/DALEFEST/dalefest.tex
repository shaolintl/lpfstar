...
like \textcolor{red}{in ???(ELF? Beluga? MMT?)} is just a sub-language of
$\lambda$Prolog.
...
sorts as well. We then instantiate the PTS to the one of Matita and we
modularly add to the kernel support for globally defined constants and
primitive inductive types, following the variant of the Calculus of Inductive
Constructions of Matita. Finally, we embed ELPI in Matita and divert all calls
to the kernel written in $\lambda$Prolog in order to test it on the arithmetic
...
\textcolor{red}{<BEGIN>}Note that the rules are syntax directed, i.e. with explicit calls to
...
The former is trivial because we reuse the $\beta$-reduction of $\lambda$Prolog for capture avoiding substitution. The two former predicates are then defined by levels: to compare two terms, we reduce both to their weak head normal forms via \verb+whd*+ and then compare the two heads. If they match, the comparison is called recursively on the subterms.
...
\textcolor{red}{Il salto fra la frase prima e addirittura l'inizio di una sottosezione è atroce. Magari iniziare la sottosezione prima di ``Consider''?}
...
machinery to wake up delaied goals that re-enter the fragment, but does not
...
elaborator.  In addition to that, the set of delayed goals implicitely kept
...
managing explicitly backtracking and goal resumption.  More that that, the automatic resumption of typing
constraints makes it impossible to obtain an ill-typed term by
instantiation: the code is correct by construction.  Last, it lets one selectively disable the undesired generative semantics of predicates like \verb+of,lt,+\ldots.
...
\item[forward reasoning during search] When propagation rules are theorems
	...
	then propagation respect the declarative, proof theoretic semantics
...
\item[code optimizations given by the meta theory] of the object
	...
	the implicit metasenv linear in the number of missing sub-proofs.
        In turns, this greatly reduces the number of checks to be performed when
...
\item[turn checking into inference]  By replacing the
	...
	\verb+leq,ltn+ one gets an elaborator that works with floating sorts~\cite{algebraic},
...
\textcolor{red}{compare equal as sets CSC:???}).  \verb+G1 => conv T1 T2+ (syntactic sugar for \verb+H1 => ... => HN => conv T1 T2+ where
...
to solving this equation written using the nabla quantifier of~\textcolor{red}{\cite{XXX}}:
...
the following one: \textcolor{red}{CSC: qua non si capisce. È un goal sospeso? È il lhs di una regola di propagazione? Usavi questa sintassi prima per le query, e non può essere una query}
...
\verb+nat_of_ord+ context.  Such example occurs very frequently
...
metavariable, turning it into a proof obligation, i.e. a sequent of the form $\Gamma \vdash \mathtt{of}~X~t$\textcolor{red}{CSC: io ho usato $\vdash$ qui e là ma forse è meglio usare sempre \texttt{?-}} where $\Gamma$ holds type declarations for variables (\verb+of x t+) or value assignments (\verb+val x t v nf+).
...
using the Abella~\cite{Abella} theorem prover to prove its correctness.
