...
conventional languages.  If we make its core operation, machine, even
more expensive, we are unlikely to obtain a working system.

Moreover we empirically observe that, in our use cases, all relevant names are
...
variables in scope of the meta variable.

What we do is to forbid the $\nabla$ in the pattern, and implicitly consider
a $\nabla$ many as the variable  in the goal are implicit. This way such injection is trivial.
...
\paragraph{Term comparison: the \texttt{sub,conv,match\_sort,match\_arr} predicates}
