...
a type-checker for Landau's Grundlagen \textcolor{red}{XXXXXXXX}.
...
kernel support for globally defined constants and primitive inductive
...
In Section~\ref{sec:problems} we pose again the same question we posed for the kernel. Is $\lambda$Prolog suitable as a very high level programming language to implement an elaborator? If not, what shall be added? We conclude that $\lambda$Prolog is not suitable and, combining and extending existing ideas in the litterature, we introduce the first Constraint Programming extension of $\lambda$Prolog. We also implemented an interpreter for the extended language that is called ELPI (Embedded Lambda Prolog Interpreter).
